...
| `google_product_category` | Static `config/category-map.json`, keyed by Zazzle's raw `product_type` | ⚠️ **OPEN — map is currently empty.** Seed with the 3 known data points from `expected_data.md` (T-Shirt → 212, Business Card → 3109, Mug → 2162); full coverage needs the ticket owner. Falls back to omitting the field (Google auto-classifies) when unmapped — this fallback is fine to keep. |
...
| `google_product_category` | No | Only when `category-map.json` has an entry (3 seeded so far) | Recommended, not hard-required — Google auto-classifies if omitted; current fallback is correct |
...
3. **`category-map.json` completion** — empty beyond the 3 seeded values; needs the ticket owner's full Zazzle-`product_type` → `google_product_category` (numeric) mapping (§4). No longer blocks the human-readable `product_type` label, which is now derived directly from `departmentName`.
